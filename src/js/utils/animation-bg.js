(function () {
  const canvas = document.createElement('canvas'),
    ...
    w = (canvas.width = outerWidth),
    h = (canvas.height = outerHeight),
    particles = [],
    properties = {
      bgColor: 'rgba(17, 17, 17, 0.35)',
      particleColor: 'rgba(220, 86, 197, 0.2)',
      particleRadius: 2,
      particleCount: 180,
  ...
  window.onresize = function () {
    (w = canvas.width = outerWidth), (h = canvas.height = outerHeight);
  };
    ...
    for (let i in particles) {
      for (let j in particles) {
        ...
        if (length < properties.lineLength) {
          ...
          ctx.lineWidth = '0.6';
          ctx.strokeStyle = 'rgba(220, 86, 197, 0.1)';
          ctx.beginPath();
          ctx.moveTo(x1, y1);
          ctx.lineTo(x2, y2);
          ctx.closePath();
